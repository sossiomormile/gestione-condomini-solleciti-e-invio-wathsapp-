...
function sourceUnits(wb,fileName){
 ...
 for(let r=0;r<rows.length;r++){
  ...
  if(name>=0){const rata=H.findIndex(x=>x==='RATA'),interno=H.findIndex(x=>x==='INTERNO'||x==='INT'||x==='INT.'),piano=H.findIndex(x=>x==='PIANO'||x==='P'),sub=H.findIndex(x=>x==='SUB'),scalaCol=H.findIndex(x=>x==='SCALA'||x==='S'),ind=H.findIndex(x=>x==='SPESE INDIVIDUALI'||x==='SPESE INDIVUALI'),indPaid=H.findIndex(x=>x==='RISCOSSE'||x==='RISCOSSIONE'),totalDue=H.findIndex(x=>x.includes('TOTALE ANNUO DA INCASSARE')||x.includes('TOTALE PERIODO DA INCASSARE')),balance=H.findIndex(x=>x.includes('TOTALE ANNUO ANCORA DA INCASSARE')||x.includes('TOTALE PERIODO ANCORA DA INCASSARE'));let mc=[];H.forEach((x,i)=>{if(MONTHS[x])mc.push([i,MONTHS[x]])});if(mc.length>12)mc=mc.slice(-12);if(rata>=0&&mc.length)h={name,rata,interno,piano,sub,scalaCol,ind,indPaid,totalDue,balance,plan:plan(mc,start)};continue}
  ...
  const monthlyPaid=h.plan.reduce((s,x)=>s+Math.max(0,num(row[x.c])),0),indDue=h.ind>=0?num(row[h.ind]):0,indPaid=h.indPaid>=0?num(row[h.indPaid]):0,totalDue=h.totalDue>=0?num(row[h.totalDue]):0,balance=h.balance>=0?num(row[h.balance]):0,monthlyDue=totalDue>0?Math.max(0,totalDue-indDue):Math.max(0,rata*12),ordinaryCredit=round2(Math.max(0,monthlyPaid-monthlyDue)+Math.max(0,indPaid-indDue)+Math.max(0,-balance));
  out.push({order:order++,sourceRow:r,name:person,canon:canon(person),scala:String(h.scalaCol>=0?(row[h.scalaCol]??''):'').trim()||scala,interno:String(h.interno>=0?(row[h.interno]??''):'').trim(),piano:String(h.piano>=0?(row[h.piano]??''):'').trim(),sub:String(h.sub>=0?(row[h.sub]??''):'').trim(),rata,unpaid,ordinaryCredit});
 }
 return out;
}
...
function structuralScore(a,b){let s=0;for(const [f,w] of [['sub',32],['scala',8],['interno',8],['piano',4]]){const x=unitNorm(a?.[f]),y=unitNorm(b?.[f]);if(x&&y&&x===y)s+=w}return s}
...
function congAssignments(wb,units){
 const byRow=new Map(units.map(u=>[u.sourceRow,{debit:0,credit:0}])),counts=new Map();for(const u of units)counts.set(u.canon,(counts.get(u.canon)||0)+1);const duplicate=new Set([...counts].filter(([,n])=>n>1).map(([c])=>c)),confidence=new Map([...duplicate].map(c=>[c,true]));
...
function congCredit(p){return round2((p?.credits||[]).filter(isCongCredit).reduce((s,x)=>s+num(x.amount),0))}
function rebuildCollapsed(result,wb,fileName){
 ...
 for(const [c,srcGroup] of groups){
  ...
  const baseDebit=round2(appGroup.reduce((s,p)=>s+congDebt(p),0)),baseCredit=round2(appGroup.reduce((s,p)=>s+congCredit(p),0)),splitDebit=round2(srcGroup.reduce((s,u)=>s+(assign.byRow.get(u.sourceRow)?.debit||0),0)),splitCredit=round2(srcGroup.reduce((s,u)=>s+(assign.byRow.get(u.sourceRow)?.credit||0),0));
