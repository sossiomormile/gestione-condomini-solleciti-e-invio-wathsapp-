...
api.addHistoryUnit(p1,'WhatsApp · 3331111111 · apertura invio',100);
const historyBefore=JSON.stringify(api.getHistoryUnit(p1));
api.setContact(p1,{phone:'',email:''});
if(api.getContact(p1).phone!=='')throw new Error('Il numero WhatsApp cancellato viene ripristinato');
if(JSON.stringify(api.getHistoryUnit(p1))!==historyBefore)throw new Error('La cancellazione del numero modifica lo storico precedente');
