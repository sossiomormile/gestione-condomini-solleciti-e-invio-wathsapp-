(()=>{
'use strict';
const REFRESH_MARK='condo_phase_ordinary_cong_neighbor_v3_saldo_done';
try{
...
function isAnchor(ev,pos){if(ev.ui===pos)return true;if(ev.ui<0&&ev.ni===pos)return true;return ev.ui===ev.ni&&ev.ni===pos}
...
function rowByNeighbors(i,target,map){if(!map.countEqual||map.contradiction||i<0||i>=target.length)return-1;const above=map.anchors.filter(x=>x<i).pop(),below=map.anchors.find(x=>x>i);if(above===undefined||below===undefined)return-1;return i}
function resolveEntry(e,i,target,map){if(!target.length)return{idx:-1,method:'',conflict:false};const ev=evidenceFor(e,target);if(ev.ui>=0){if(ev.ni>=0&&ev.ni!==ev.ui&&ev.ui!==i)return{idx:-1,method:'',conflict:true};return{idx:ev.ui,method:'piano/interno univoci',conflict:false}}if(ev.ni>=0)return{idx:ev.ni,method:'nome esatto',conflict:false};const ri=map?rowByNeighbors(i,target,map):-1;return ri>=0?{idx:ri,method:'riga confermata da condomini sopra e sotto',conflict:false}:{idx:-1,method:'',conflict:false}}
...
p._v7=p._v7||{};p._v7.conguaglioMatched=p._v7.conguaglioMatched||[];p._v7.conguaglioMatched.push(sn+' riga '+e.sheetRow+' → '+method+' · valore da '+e.balanceSource)}
if(ambiguous)r.warnings.push('DA VERIFICARE: '+ambiguous+' conguaglio/i in '+sn+' presenta/no dati identificativi in conflitto; nessuna attribuzione automatica.');if(unresolved)r.warnings.push('DA VERIFICARE: '+unresolved+' conguaglio/i in '+sn+' non ha/hanno una corrispondenza univoca dimostrabile; nessuna attribuzione automatica.');if(target.length&&map&&!map.countEqual)r.warnings.push('Controllo '+sn+': numero righe diverso da Incassi; abbinamento per riga disabilitato.');if(target.length&&map&&map.contradiction)r.warnings.push('Controllo '+sn+': ordine strutturale non coerente con Incassi; abbinamento per riga disabilitato.')}}r.warnings=[...new Set(r.warnings)];return r}
