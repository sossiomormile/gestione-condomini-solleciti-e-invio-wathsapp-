...
const SYNC_KEY='condo_drive_sync_state_v8';
const ARCHIVE_KEY='condo_archive_v5';
const UPDATE_KEY='condo_last_update_v5';
const TX_KEY='condo_drive_full_refresh_tx_v4';
...
function sig(r){return[r.balance?.id||'',r.balance?.modifiedTime||'',r.balance?.name||'',r.rule||'AUTO','ENGINE_V7_FULLREFRESH4_CERT1'].join('|')}
...
function renderRows(rows,updated=0){const area=ensureArea(),ok=rows.filter(r=>r.state==='ok');area.innerHTML='<div>Condomini Drive attivi: '+rows.length+' · ricaricati e certificati integralmente: '+updated+'</div><select id="driveCondoSelect"></select><button id="driveOpenCondo">APRI CONDOMINIO</button>';const sel=document.getElementById('driveCondoSelect');ok.forEach((r,i)=>{const o=document.createElement('option');o.value=String(i);o.textContent=r.folder.name;sel.appendChild(o)});document.getElementById('driveOpenCondo').onclick=async()=>{const r=ok[Number(sel.value||0)];try{window.condoSetFolderTitle?.(r.folder.name);window.condoSourcePath=sourcePath(r);const f=await downloadExcel(r.balance);await parseFile(f);statusMsg('Aperto: '+r.folder.name+' · '+r.balance.name)}finally{window.condoSourcePath='';window.condoClearFolderTitle?.()}}}
async function rebuildAll(rows){
 const checker=window.condoSelfCheckV1;if(!checker?.auditFile||!checker?.finalize)throw new Error('Modulo autocollaudo V1 non disponibile: aggiornamento bloccato.');if(!window.condoDeepCheckV2)throw new Error('Autocollaudo profondo V1 non disponibile: aggiornamento bloccato.');if(!window.condoUpdateCertificationV1?.active)throw new Error('Certificazione estesa AGGIORNA non disponibile: aggiornamento bloccato.');checker.resetBatch();const ok=rows.filter(r=>r.state==='ok'),newState={};let updated=0;
 for(let i=0;i<ok.length;i++){
  const r=ok[i];statusMsg('Ricarico, verifico e certifico '+r.folder.name+' · '+r.balance.name+' ('+(i+1)+'/'+ok.length+')…');window.condoSetFolderTitle?.(r.folder.name);window.condoSourcePath=sourcePath(r);
  try{const f=await downloadExcel(r.balance),src=document.getElementById('source');if(src)src.textContent='';if(typeof current!=='undefined')current=[];await parseFile(f);const parsedOk=src?String(src.textContent||'').includes(r.balance.name):(typeof current!=='undefined'&&Array.isArray(current));if(!parsedOk)throw new Error('Analisi non completata per '+r.folder.name+' · '+r.balance.name);const audit=await checker.auditFile(f,Array.isArray(current)?current:[],r.folder.name,r.balance.name);checker.add(audit);if(!audit.ok)throw new Error('Certificazione non superata per '+r.folder.name+': '+audit.errors.slice(0,3).join(' | '));newState[r.folder.id||r.folder.name]=sig(r);updated++}finally{window.condoSourcePath='';window.condoClearFolderTitle?.()}
 }
 const auditSummary=checker.finalize(ok.length);if(!auditSummary.ok)throw new Error('Certificazione complessiva non superata: '+auditSummary.errors.slice(0,3).join(' | '));localStorage.setItem(SYNC_KEY,JSON.stringify(newState));return{updated,audit:auditSummary};
}
async function doRefresh(){
 const root=await findFolder(),rows=await scanCondomini(root),blocked=rows.filter(r=>r.state!=='ok'||r.validated!==true);if(blocked.length)throw new Error('Aggiornamento annullato: '+blocked.length+' condomini non hanno una sorgente bilancio certificata ('+blocked.map(r=>r.folder?.name||'?').join(', ')+'). Nessun dato viene confermato.');const tx=beginFullRefreshTransaction();
 try{const result=await rebuildAll(rows);if(result.updated!==rows.length)throw new Error('Ricostruzione incompleta: '+result.updated+'/'+rows.length);const archiveCheck=verifyArchiveMatchesDrive(rows,tx.startedAt);if(!archiveCheck.ok)throw new Error('Archivio ricostruito non coincide con Drive: '+archiveCheck.missing.length+' mancanti, '+archiveCheck.extra.length+' residui, '+archiveCheck.stale.length+' non aggiornati.');const protectedCheck=verifyProtectedData(tx);if(!protectedCheck.ok)throw new Error('Contatti WhatsApp, storico o configurazioni sono cambiati durante AGGIORNA: operazione annullata.');result.audit.memory={freshArchive:true,archiveEntries:archiveCheck.entries,archiveExpected:archiveCheck.expected,archiveMatchesDrive:true,protectedDataUnchanged:true};commitFullRefreshTransaction();renderRows(rows,result.updated);window.condoSelfCheckV1?.renderBatchSummary(result.audit);statusMsg('Aggiornamento certificato completato · '+result.updated+' condomini riletti integralmente da Drive e tutti i controlli superati.');return result}
...
async function refreshDrive(){const btn=document.getElementById('folderBtn');try{if(btn){btn.disabled=true;btn.textContent='CERTIFICAZIONE…'}statusMsg('Collegamento a Google Drive…');await ensureToken();await doRefresh()}catch(e){statusMsg(e.message,true);alert(e.message)}finally{if(btn){btn.disabled=false;btn.textContent='AGGIORNA'}}}
function setup(){restoreToken();recoverInterruptedTransaction();const old=document.getElementById('folderBtn');if(!old)return;const btn=old.cloneNode(true);old.replaceWith(btn);btn.removeAttribute('onclick');btn.textContent='AGGIORNA';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();refreshDrive()},{capture:true});const p=btn.closest('.card')?.querySelector('p.muted');if(p)p.innerHTML='Premi <b>AGGIORNA</b>: il gestionale rilegge integralmente ogni bilancio certificato da Drive, ricostruisce l’archivio e prima di confermarlo esegue controlli incrociati su sorgente, nominativi, unità, righe Incassi, rate, mesi futuri, centesimi, totali lordi e conguagli. Se anche un solo controllo fallisce, ripristina automaticamente i dati precedenti.'}
const publicApi={beginFullRefreshTransaction,rollbackFullRefreshTransaction,commitFullRefreshTransaction,recoverInterruptedTransaction,verifyFreshArchive,verifyArchiveMatchesDrive,verifyProtectedData,resetRuntimeViewAfterRollback,doRefresh,rebuildAll,sourceRules:SOURCE_RULES,keys:{ARCHIVE_KEY,SYNC_KEY,UPDATE_KEY,TX_KEY}};
